...
const FeaturedPosts = (props: {
	...
	return (
		<div className="blog-featured-area mx-auto border-b bg-slate-50 dark:border-slate-800 dark:bg-black">
			<div className="blog-featured-container container mx-auto grid grid-cols-1 gap-8 px-4 py-4 md:grid-flow-col md:grid-cols-2 md:grid-rows-2 xl:grid-cols-3 xl:px-10 xl:py-10 2xl:px-24 2xl:py-10">
				{posts.map((post, index) => {
					...
					return (
						<div
							key={post.id}
							className={twJoin(
								'blog-article-card col-span-1',
								isFirstPost ? 'md:row-span-2 xl:col-span-2' : '',
							)}
						>
							<Link
								href={postURL}
							...
							<h1
								className={twJoin(
									'blog-article-card-title font-heading mx-4 mb-3 block font-extrabold text-slate-900 hover:opacity-75 dark:text-slate-100',
							...
							{isPinnedToBlog && (
								<div className="blog-article-card-label font-heading mx-4 mb-1 flex flex-row items-center break-words font-medium leading-snug text-blue-600 dark:text-blue-500">
									<span>Pinned</span>
							...
							<div className="blog-article-card-author-strip mx-4 flex flex-row flex-wrap items-center">
								...
								>
									<Image
										...
										width={72}
										height={72}
										src={resizeImage(post.author.profilePicture || DEFAULT_AVATAR, {
											w: 72,
											h: 72,
											c: 'face',
										})}
				...
				{[...new Array(padding)].map(() => (
					<div
						key={`padding-${cuid()}`}
						className="col-span-1 row-span-1 rounded-lg bg-slate-200 dark:bg-slate-800"
					/>
				))}
			</div>
		</div>
